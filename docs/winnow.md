...
GDB, unblob, Malwoverview, Anya, and Angr are plain command-line tools whose entire result is printed to standard output, not a GUI window - BinSifter opens them inside a real terminal (whichever of `x-terminal-emulator`/`gnome-terminal`/`konsole`/`xfce4-terminal`/`xterm` is found on PATH) instead of a background launch that would otherwise silently discard that output. That terminal also stays open after the command exits and prints its exit status - a fast crash (e.g. a broken system `gdb`) is now visible instead of the window flashing shut before you can read anything.
...
**Most of these tools also install themselves automatically - you don't have to do any of this by hand.** As of 2026-09-03, on every startup BinSifter checks whether each of PE-bear/Anya/DIE/Cutter/Angr/unblob/Malwoverview/Ghidra is already findable (your "Path to tools"/"Path to Ghidra" directory, your normal shell PATH, or a copy BinSifter downloaded itself previously); anything missing gets downloaded and installed automatically in the background, without ever blocking the window from opening or a scan from running. Everything is a plain per-user download - never `sudo`, never a system package manager - so nothing here needs elevated permissions. All of these are installed to `$XDG_DATA_HOME/binsifter-winnow/AutoInstalledTools` (or `~/.local/share/binsifter-winnow/AutoInstalledTools` if `$XDG_DATA_HOME` isn't set) - deliberately NOT under this same page's `~/BinSifter Winnow` Reports/Settings location, since that name's space breaks some pip-installed tools' own launcher scripts (a raw kernel shebang line has no way to represent a space in a path at all).
...
- **unblob** and **Malwoverview** are both PyPI packages - `pipx install unblob` / `pipx install malwoverview`. unblob's full format coverage additionally wants a handful of system extraction tools (`p7zip-full`, `unar`, `e2fsprogs`, `zstd`, `lz4`, optionally `sasquatch` for squashfs) installed via your distro's package manager.
