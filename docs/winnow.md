...
GDB, Binwalk, and Malwoverview are plain command-line tools with no window of their own - BinSifter opens them inside a real terminal (whichever of `x-terminal-emulator`/`gnome-terminal`/`konsole`/`xfce4-terminal`/`xterm` is found on PATH) instead of a background launch that would otherwise silently do nothing.
