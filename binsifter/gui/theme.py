...
def detect_os_dark_mode() -> bool:
    """Port of Test-SystemDarkMode (BinSifter-Rowan.ps1, lines 36-46).

    Reads the same registry value Rowan reads
    (HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize
    \\AppsUseLightTheme, 0 = dark, nonzero/absent = light) so both variants
    agree on what "the OS theme" means. Checked once at startup, not live -
    changing the OS theme while either variant is running requires a
    relaunch to pick up.

    Falls back to light mode if `winreg` isn't importable (non-Windows
    platforms) or the key/value is missing or unreadable for any other
    reason - never raises.
    """
    try:
        import winreg
