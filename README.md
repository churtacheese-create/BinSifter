...
| **[Ingot](docs/ingot.md)** | Rust backend service + web UI (the actual cross-platform variant) | **In progress.** Phases 1-5 done: hashing/entropy, NSRL, blocklist, imphash, SSDEEP, YARA + severity + MITRE ATT&CK, capa + FLOSS + IOC extraction, SSDEEP/imphash clustering + draft rules, disposition tracking, 37-column CSV reports, and a live web UI, built and validated. Remaining detection stages (Authenticode, archives) porting one at a time. |
...
- **[Ingot](docs/ingot.md)** - Rust, needs a toolchain (1.80+) to build: `cd ingot && cargo run`. Phases 1-5 so far (hashing/entropy, NSRL, blocklist, imphash, SSDEEP, YARA + severity + MITRE ATT&CK, capa + FLOSS + IOC extraction, SSDEEP/imphash clustering + draft rules, disposition tracking, CSV reports, web UI).
