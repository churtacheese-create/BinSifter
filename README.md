...
| **[Ingot](docs/ingot.md)** | Rust backend service + web UI (the actual cross-platform variant) | **Released ([v0.1.0](https://github.com/churtacheese-create/BinSifter/releases/tag/ingot-v0.1.0)).** The full phased port is done and validated: archive expansion, hashing/entropy, Authenticode, NSRL, blocklist, imphash, SSDEEP, YARA + severity + MITRE ATT&CK, capa + FLOSS + IOC extraction, SSDEEP/imphash clustering + draft rules, disposition tracking, 37-column CSV reports, a live web UI, a right-click quick-launch tool menu (auto-scoped to the host OS) + Ghidra headless + AI-analysis export. Self-contained per-OS release binaries (Linux, Windows, macOS - Apple silicon and Intel) are published on the `ingot-v*` tags. Known gaps: catalog signature verification, SSDEEP cluster history (neither is in the other variants either). |

If you're on Windows, use Rowan. If you're on Linux, use Winnow. Ingot is the one variant meant to run anywhere via a browser instead of a desktop GUI toolkit; grab a prebuilt binary from its [releases page](https://github.com/churtacheese-create/BinSifter/releases?q=ingot-v) or build it from source on any of the three.

**NSRL caching (applies to all three variants):** the first scan against a given NSRL hash set builds a cached, memory-mapped index from it - a one-time cost that scales with the NSRL file's size, not the number of files being scanned (around 30 minutes for a full ~430-million-hash NSRL RDS set, measured directly). Every scan after that against the same NSRL file and Report Directory loads the cache directly (well under a second) instead of re-parsing it, so don't be alarmed if the very first scan against a new NSRL set takes noticeably longer than every scan after it. The cache lives under `<ReportDirectory>/.bsifter-nsrl-cache/` and rebuilds automatically if the source NSRL file's size or modified date changes, so switching Report Directory or NSRL file costs one more full rebuild, not a lasting slowdown.
...
- **[Ingot](docs/ingot.md)** - download a prebuilt binary for your OS from the [releases page](https://github.com/churtacheese-create/BinSifter/releases?q=ingot-v) and run `ingot` (`ingot.exe` on Windows) - no toolchain needed. To build from source instead, a Rust toolchain (1.93+) is all it takes: `cd ingot && cargo run`. The full phased port is done (archive expansion, hashing/entropy, Authenticode, NSRL, blocklist, imphash, SSDEEP, YARA + severity + MITRE ATT&CK, capa + FLOSS + IOC extraction, SSDEEP/imphash clustering + draft rules, disposition tracking, CSV reports, web UI, OS-scoped quick-launch tool menu + Ghidra headless + AI-analysis export).
