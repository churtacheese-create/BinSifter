...
| **[Ingot](docs/ingot.md)** | Rust backend service + web UI (the actual cross-platform variant) | **Feature-complete, not yet released.** The full phased port is done and validated: archive expansion, hashing/entropy, Authenticode, NSRL, blocklist, imphash, SSDEEP, YARA + severity + MITRE ATT&CK, capa + FLOSS + IOC extraction, SSDEEP/imphash clustering + draft rules, disposition tracking, 37-column CSV reports, a live web UI, a right-click quick-launch tool menu (auto-scoped to the host OS) + Ghidra headless + AI-analysis export. A per-OS release-binary workflow and CI are in place; the first published release is pending their first run. Known gaps: catalog signature verification, SSDEEP cluster history (neither is in the other variants either). |

If you're on Windows, use Rowan. If you're on Linux, use Winnow. Ingot is the one variant meant to run anywhere via a browser instead of a desktop GUI toolkit; it builds and runs today from source on all three, with per-OS release binaries wired up but not yet published.
...
- **[Ingot](docs/ingot.md)** - Rust, needs a toolchain (1.93+) to build: `cd ingot && cargo run`. The full phased port is done (archive expansion, hashing/entropy, Authenticode, NSRL, blocklist, imphash, SSDEEP, YARA + severity + MITRE ATT&CK, capa + FLOSS + IOC extraction, SSDEEP/imphash clustering + draft rules, disposition tracking, CSV reports, web UI, OS-scoped quick-launch tool menu + Ghidra headless + AI-analysis export). Per-OS release binaries build via GitHub Actions (`ingot-v*` tags); none published yet.
