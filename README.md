...
| **[Ingot](docs/ingot.md)** | Rust backend service + web UI (the actual cross-platform variant) | **In progress.** Phases 1-7 done: archive expansion, hashing/entropy, Authenticode, NSRL, blocklist, imphash, SSDEEP, YARA + severity + MITRE ATT&CK, capa + FLOSS + IOC extraction, SSDEEP/imphash clustering + draft rules, disposition tracking, 37-column CSV reports, a live web UI, and a right-click quick-launch tool menu (auto-scoped to the host OS) + Ghidra headless + AI-analysis export, built and validated. Remaining: catalog signature verification, packaging. |

If you're on Windows, use Rowan. If you're on Linux, use Winnow. Ingot is the one variant meant to run anywhere via a browser instead of a desktop GUI toolkit; it's not yet packaged for end users but builds and runs today from source.
