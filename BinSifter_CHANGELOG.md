...
## Settings-page "Check for updates" (2026-09-15)
- `$Global:BinSifterVersion` reintroduced (`'2.0.8'`, matching Winnow's `__version__`) and shown on the About page - the 2026-08-19 entry above retired the old `$AppVersion` display alongside dropping the version-in-filename convention; that reasoning no longer applies now that real `v*` GitHub releases exist and this update feature needs something real to compare against.
- New "Update" section on the Settings page: "Check for updates" queries this repo's GitHub releases (filtered to the `v*` tag line Rowan shares with Winnow - Ingot's separate `ingot-v*` line is never matched), and, if a newer version exists, offers to download and install it. Confirming downloads `BinSifter-Rowan.ps1` and its three image assets straight from the tagged commit's raw content (no compile step, so the plain repo files ARE the release) into a staging folder, then closes BinSifter so a small detached helper script can copy the staged files over the real install and relaunch it.
- Deliberately scoped to the plain-script install (run via `pwsh.exe`/`powershell.exe` - not locked while running) - the portable PS2EXE-compiled `.exe` is a real locked binary and gets a "download it yourself" message instead of a separate replace mechanism built for a disposable/no-install format.
- Winnow got the equivalent feature the same day, but check-and-guide only (no self-overwrite) - it's a root-owned Linux package (`.deb`/`.rpm`/`.pkg.tar.zst`), and rewriting those files outside `dpkg`/`rpm`/`pacman` would corrupt their own integrity tracking. Ingot (a single portable binary, no installer) got a real automatic self-update instead, via the `self-replace` crate. See `ingot/IMPLEMENTATION_PLAN.md` for the full cross-variant design notes.

## v2.0.9
- Ghidra's GUI never actually opened from the Results-grid quick-launch action - the chain script that runs `analyzeHeadless` then `ghidraRun` was missing `call` before each nested `.bat` invocation, which in `cmd.exe` batch scripting transfers control into the nested script permanently instead of returning - so the whole chain silently died the instant `analyzeHeadless.bat` finished, well before `ghidraRun` ever ran. Fixed, then found two further bugs re-testing against a real filename containing parentheses: `cmd.exe`'s own parser treats `(`/`)` as command-grouping syntax even inside a quoted argument, AND Ghidra's own bundled `analyzeHeadless.bat` independently re-breaks on the same parentheses in its own wildcard-expansion and project-name parsing, even once our own call line was `set`-protected. Real fix: stage the import target under a generated, special-character-free filename, and sanitize the auto-generated project name, instead of ever handing Ghidra a risky value.
- The quick-launch menu's other Ghidra-adjacent issues from the same testing round: DIE and Sigcheck resolving to the wrong/non-interactive build, CFF Explorer never receiving the target path (its own command line is reserved for Lua scripting - now copies the path to the clipboard instead, same as the other variants), and every quick-launch tool opening behind the main window (added a Z-order-based foreground raise; deliberately not `AttachThreadInput`, which silently crashed DIE).
- New Settings-page "Check for updates" feature (see the entry above) - real end-to-end verified this round: real GitHub API call, real download of all 4 release files, real detached-helper file replace, and a real relaunch, confirmed both via an isolated test harness and a live click-through.
