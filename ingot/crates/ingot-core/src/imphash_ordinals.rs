//! Ordinal-to-name tables for imphash, matching the `ordlookup` package that
//! `pefile` uses (tables: oleaut32.dll, ws2_32.dll, wsock32.dll). Generated -
//! do not edit by hand. Consulted only when a PE imports a symbol by ordinal;
//! any ordinal not found here becomes `ord<N>`, exactly as
//! `ordlookup.formatOrdString` does.

/// `(dll_name_lowercase, &[(ordinal, name)])`, ordinals ascending.
pub(crate) static ORDINAL_TABLES: &[(&str, &[(u16, &str)])] = &[
    (
        "oleaut32.dll",
        &[
            (2, "SysAllocString"),
