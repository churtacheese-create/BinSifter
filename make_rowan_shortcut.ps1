...
# isn't just cosmetic: Rowan's $BinSifterRoot resolution falls back to
# whatever the process's working directory happens to be if $PSScriptRoot
# comes back empty (see BinSifter-Rowan_v1.3.0-beta.1.ps1 lines ~1694-1718), and
...
$scriptPath = Join-Path $repoDir "BinSifter-Rowan_v1.3.0-beta.1.ps1"
...
if (-not $pwshCommand) {
    throw "Could not find pwsh.exe (PowerShell 7+) on PATH - Rowan requires it (uses SHA256.HashData, a .NET 5+ API not available under legacy powershell.exe). Install PowerShell 7 or locate pwsh.exe manually."
}
...
$shortcut.Description = "BinSifter Rowan (PowerShell / beta.1)"
$shortcut.Save()
